...
\lstdefinestyle{hask}{
  ...
  emphstyle={[8]\color{Aquamarine}\textbf},
}
...
\begin{frame}[fragile]{System $F_c$}
  Currently, Haskell as of GHC 7.10.2 doesnot have true type operators. Achieves type-level programming through \texttt{type families} and equality on \htycon{Types}. This axis of extension on $\lambda2$ is termed \text{System} $F_c$.

  \ \\
  \pause
   Haskell is not truely dependent either because of the strong distinction between \hvalcon{Values} and \htycon{Types}. But with a handful of \texttt{language extensions} and the current \hkind{Kind} system, we are ready to fake dependent types in Haskell.
\end{frame}
...
\begin{frame}[fragile]{Introducing Constraint}
  Haskell has $typeclasses$ that very loosely resemble interfaces in OOP. A basic \hclass{Typeclass} consists of a collection of function signatures for a \htycon{Type} to implement. Afterward, this \hclass{Typeclass} instance can be used to provide \texttt{contexts} for functions.
  \pause
  \begin{lstlisting}[style=hask]
...
\begin{frame}[fragile]{Head of the List}
  \texttt{head} returns the first element of the list:
  \begin{lstlisting}[style=hask]
  ...
  \texttt{Elm} updated to:
  \begin{lstlisting}[style=hask]
    head :: @tc[@a@tc]@ -> Maybe a
    head @vc[]@     = Nothing
    head (x@vc:@xs) = Just x
  \end{lstlisting}
  ...
  Alternatively:
  \begin{lstlisting}[style=hask]
    head :: @tc[@a@tc]@ -> Either String a
    head @vc[]@     = Left "empty list"
    head (x@vc:@xs) = Right x
  \end{lstlisting}
\end{frame}
  ...
  \begin{itemize}
    \item Total functional languages
      \begin{itemize}
        ...
        \item distinguish between \hkeyword{data} and \hkeyword{codata}
      \end{itemize}
